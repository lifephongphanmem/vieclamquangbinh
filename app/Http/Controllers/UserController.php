...
class UserController extends Controller
{
	...
	public function index_nn(Request $request)
	{
		...
		if ($inputs['phanloaitk'] == 1) {
			$m_danhmuc = danhmuchanhchinh::join('dmdonvi', 'dmdonvi.madiaban', 'danhmuchanhchinh.id')
				...
				->get();
			$ds_huyen = $m_danhmuc->where('capdo', 'H');
			if (isset($inputs['madv'])) {
				$mahuyen = $ds_huyen->where('madv', $inputs['madv'])->first()->maquocgia;
				$model_hc = $model_hc->where('parent', $mahuyen);
			} else {
				$inputs['madv'] = null;
			}
	...
	public function update_tk(Request $request, $id)
	{
		...
		Company::where('user', $model->id)->update(['madv' => $inputs['madv']]);
		// $model->update($inputs);
		if ($model->phanloaitk == 1) {
